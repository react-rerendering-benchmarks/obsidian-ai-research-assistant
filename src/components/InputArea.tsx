...
const countInput = (input: string, countType: string, tokenType: TokenCounterType = DEFAULT_TOKEN_TYPE): number => {
  switch (countType) {
    case 'tokens':
...
const InputArea = memo(({
  ...
  const text = useRef('');
  const [debouncedText] = useDebounce(text.current.value, delay);
  ...
  const renderCount = memo((): React.ReactElement => {
    if (typeof countType === 'undefined') {
      return <></>;
    }
    return <div className={`ai-research-assistant__input-area__toolbar${typeof countAlign !== 'undefined' ? ` ai-research-assistant__input-area__toolbar--${countAlign}` : ''}${typeof countPosition !== 'undefined' ? ` ai-research-assistant__input-area__toolbar--${countPosition}` : ''}`} style={{
      ...
      textAlign: countAlign
        ...
        {warning ? <div className="ai-research-assistant__input-area__counter__warning" style={{
  ...
  return <div className="ai-research-assistant__input-area">
      {countPosition === 'top' ? renderCount() : <></>}
      <div className={`ai-research-assistant__input-area__input${typeof label !== 'undefined' ? ` ai-research-assistant__input-area__input--label-${labelPosition}` : ''}`}>
        {typeof label !== 'undefined' ? <label className="ai-research-assistant__input-area__input__label" htmlFor={id}>
            {label}
          </label> : <></>}
        {type === 'textarea' ? <textarea id={id} name={name} placeholder="Type your message here" onChange={handleChange} value={value} required={required} disabled={disabled} aria-label={a11yLabel} /> : <input id={id} name={name} placeholder="Type your message here" ref={text} required={required} disabled={disabled} aria-label={a11yLabel} />}
      </div>
