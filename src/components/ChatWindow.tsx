...
const ChatWindow = ({
  ...
  const renderConversation = memo((): React.ReactElement[] => {
    return conversation.messages.length > 0 ? conversation.messages.map((message, index) => {
      return <ChatBubble key={`message-${message.id}` ?? `message-${index}`} message={message} conversation={conversation} />;
    }) : [<React.Fragment key="no-results"></React.Fragment>];
  });
  useEffect(() => {
    autoSaving.current = plugin.autoSaving;
  }, [plugin.autoSaving]);
  return <div className="ai-research-assistant__conversation"
  // @ts-expect-error
  ref={scrollRef} {...containerProps}>
      ...
      {renderConversation()}
      {latestMessageContent !== null && <ChatBubble message={{
